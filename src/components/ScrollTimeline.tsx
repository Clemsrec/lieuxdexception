...
export default function ScrollTimeline({ events }: { events: TimelineEvent[] }) {
 ...
 return (
  <>
   ...
   <m.div
    className="fixed w-8 h-8 border-2 border-accent/50 pointer-events-none z-50 mix-blend-difference hidden lg:block"
    animate={{
     x: cursorPos.x - 16,
     y: cursorPos.y - 16,
    }}
   ...
   <div className="fixed right-4 md:right-8 top-1/2 -translate-y-1/2 z-40 flex-col gap-4 hidden lg:flex">
    {events.map((event, index) => (
     <m.button
      ...
      whileTap={{ scale: 0.9 }}
     >
      <div className={`w-3 h-3 transition-all border-2 ${
       activeIndex === index 
      ...
      {/* Tooltip */}
      <div className="absolute right-6 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap bg-white text-charcoal-800 px-3 py-1 text-sm hidden md:block shadow-lg">
