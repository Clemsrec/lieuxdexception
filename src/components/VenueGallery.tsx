   ...
   >
    {displayImages.map((image, index) => (
     <m.button
      ...
      className="group relative aspect-square overflow-hidden bg-stone/20 cursor-pointer"
      variants={{
     ...
     >
      ...
      {/* Numéro de la photo */}
      <div className="absolute bottom-2 right-2 bg-primary/80 text-white text-xs px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity">
   ...
   <AnimatePresence>
    {lightboxOpen && (
     ...
     <button
      ...
      className="absolute top-4 right-4 z-50 bg-white/10 hover:bg-white/20 text-white p-3 transition-colors"
     ...
     {/* Compteur */}
     <div className="absolute top-4 left-4 z-50 bg-white/10 text-white px-4 py-2 text-sm font-medium">
      ...
      <div className="relative max-w-content max-h-full w-full h-full">
       <div className="flex flex-col items-center justify-center w-full h-full">
        <Image
         ...
         quality={95}
        />
        <div className="absolute bottom-8 text-white text-sm bg-black/50 px-3 py-1">
         {venueName}
        </div>
       </div>
      </div>
     ...
     {displayImages.length > 1 && (
      <button
       ...
       className="absolute left-4 top-1/2 -translate-y-1/2 z-50 bg-white/10 hover:bg-white/20 text-white p-4 transition-colors"
     ...
     {displayImages.length > 1 && (
      <button
       onClick={goToNext}
       className="absolute right-4 top-1/2 -translate-y-1/2 z-50 bg-white/10 hover:bg-white/20 text-white p-4 transition-colors"
